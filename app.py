...
def get_tdcc_big_holders(target_id):
    try:
        ...
        # 篩選特定個股與持股分級 15 (千張以上大戶)
        df_stock = df[(df['證券代號'].astype(str) == str(target_id)) & (df['持股分級'] == 15)].copy()
        
        if not df_stock.empty:
            df_stock['資料日期'] = pd.to_datetime(df_stock['資料日期'].astype(str))
            df_stock['佔集保總股數%'] = pd.to_numeric(df_stock['佔集保總股數%'], errors='coerce')
            df_stock = df_stock.sort_values('資料日期')
            return df_stock
    except Exception:
        pass
    return pd.DataFrame()
...
try:
    ...
    if df.empty:
        ...
        if not big_holders.empty:
            latest_ratio = big_holders['佔集保總股數%'].iloc[-1]
        ...
        if not big_holders.empty:
            fig.add_trace(go.Scatter(
                x=big_holders['資料日期'], y=big_holders['佔集保總股數%'],
